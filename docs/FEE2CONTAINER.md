...
Zusätzlich erhält jedes neu erzeugte Containerobjekt eigene versionierte Properties im `TagComponent.TagEntries`: Schema, Generator, stabile Container-ID, Containertyp und Erstellzeitpunkt. Neue Provenienz wird ausdrücklich nicht mehr im `MarkComponent` gespeichert. Historische Mark-Tokens werden nur lesend unterstützt, damit vorhandene Projekte rückwärtskompatibel bleiben. Mehrere Objekte derselben Container-ID – beispielsweise Logik, Sensor und technische Move-Objekte – werden beim Rücklesen zu genau einem Container zusammengeführt. Die vollständige, prüfsummengeschützte Root-Provenienz bleibt die autoritative Round-Trip-Quelle.
