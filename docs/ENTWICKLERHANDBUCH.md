...
Ein RDP-Profil darf ausschließlich Ziel-PC, Benutzer, Monitorwahl und Abfragemodus enthalten. Der einzige Kennwortprovider ist `IUserCredentialConfigurationService.GetRemoteDesktopPassword`; `WindowsTemporaryRemoteCredentialStore` reicht den Wert über `ProcessStartInfo.ArgumentList` an `cmdkey`, protokolliert ihn nie und entfernt den Zieleintrag verzögert. Produktive Kennwörter dürfen nicht als Literal ergänzt werden. Die IBN-Einzeldatei besitzt ausschließlich die dokumentierten, ungültigen Platzhalter `12345`/`67890`; eine Ersetzung durch echte Werte ist ohne Rückkehr zu einem geschützten Store unzulässig.
...
Per-Objekt-Provenienz verwendet `ContainerObjectProvenance` und die namespaced `vibn.container-object.*`-Einträge der FEE-Property `TagComponent.TagEntries`. Neue Objekte werden nicht über `MarkComponent` gekennzeichnet. Der Reader behält alte Mark-Tokens ausschließlich als Migrationsfallback und gruppiert mehrere Objekte derselben Container-ID wieder zu einem fachlichen Container. Tag-Properties sind Metadaten und dürfen eine ausdrücklich bestätigte fachliche Generierung nicht nach bereits erfolgter Objekterzeugung abbrechen: `FeeTagPropertyStore.TryWriteAndVerifyAsync` liefert stattdessen ein Warnresultat, der BasicFrame wiederholt den Schreibversuch nach dem ersten Send, und verbleibende Fehler werden im UI-/Dateilog als Einschränkung für FEE2Container ausgewiesen.
