...
Die kompakte Übersicht für Programmbausteine und Datentypen steht am unteren Seitenende. **Achsen nur lesen** verändert das Projekt nicht und füllt **Gefundene Achsen**. **Konfigurieren + AxisDB/AxisFC erzeugen** bearbeitet ausschließlich die ausgewählten gefundenen Achsen und verschiebt nur vollständig erfolgreiche Achsen in **Konfigurierte Achsen**. Bereits konfigurierte Achsen besitzen dort eine eigene Auswahl; sie werden ohne erneutes Schreiben ihrer Parameter in AxisDB/AxisFC aufgenommen. Die eigene **AxisDB/AxisFC-Ablagewurzel** ist ein Projekt- oder Kundenordner. Das Tool erzeugt darin `_Programm\Axis\AxisDB.xml` und `_Programm\Axis\AxisFC.xml`. Das entspricht der bisherigen Avalonia-Methode `VicoscanAxis`; der dort fest eingebaute Fallback `C:\Treiber\VICO_Tool\Files\AxisBibBasic` wird im VIBN Tool bewusst nicht stillschweigend verwendet. Import und Projektspeicherung bleiben getrennte, ausdrücklich beschriebene Aktionen. Die Achsen werden nacheinander verarbeitet, sodass Fortschrittsbalken und Log Achsname und Parameterergebnis nachvollziehbar anzeigen.
...
Dieses Quality Gate ist ein statischer Build-Test. Ein automatischer HMI-/Ablauftest wie **Taste Einrichten drücken → Zylinderausgang wird gesetzt → Eingang Work/Home kommt innerhalb eines Zeitfensters** benötigt drei zusätzliche, projektspezifische Laufzeitverbindungen: WinCC Runtime beziehungsweise einen freigegebenen HMI-Testtreiber, PLCSIM Advanced oder eine Test-PLC sowie FEE/Emulate3D/EKS als Simulationsadapter. Ein belastbarer Ausbau sollte je Testfall Startzustand, Schreibrecht, Stimulus-Tag, erwartete PLC-/HMI-Tags, Timeout, Diagnose bei fehlender Rückmeldung und eine garantierte Rücksetzung definieren. Ohne diese Laufzeitadapter kann das Tool Testfälle aus dem ContainerFile vorbereiten, darf ihre Ausführung aber nicht als bestanden melden. Sinnvolle Zwischenstufen sind HMI- und PLC-Compile, Symbol-/Adresskonsistenz, fehlende Bausteinaufrufe, nicht verwendete Tags, Hardware-/I/O-Überlappungen und ein read-only Vergleich der projektierten HMI-Tags gegen das PLC-Interface.
