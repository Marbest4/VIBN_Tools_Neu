...
namespace VIBN_Tools.GlobalClasses.FeeObjects
{
    public class FeeInterface : FeeAbstractObject, IPlausibilityCheck<List<FeeBasicFrame>>
    {
        ...
        public async static Task<List<FeeInterface>> GetAllInterfacesAsync()
        {
            ...
            var interfaceTasks = interfaceGuids.Select(async guidText =>
            {
                await throttle.WaitAsync();
                try
                {
                    var guid = Guid.Parse(guidText);
                    var properties = await Services.ApiInstance.Interface.GetInterfacePropertiesAsync(guid);

                    var providerGuidText = properties.FirstOrDefault(x => x.PropertyName == "InterfaceProviderGuid")?.PropertyValue;
                    FeeInterface tempInterface = new FeeInterface()
                    {
                        Guid = guid,
                        Name = properties.FirstOrDefault(x => x.PropertyName == "InterfaceName")?.PropertyValue,
                        ProviderGuid = Guid.TryParse(providerGuidText, out var providerGuid) ? providerGuid : Guid.Empty,
            ...
            interfaceList.AddRange(await Task.WhenAll(interfaceTasks));
